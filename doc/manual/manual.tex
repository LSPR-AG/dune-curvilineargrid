...
grid manager \texttt{dune-curvilineargrid} for curvilinear tetrahedral elements up to $5^{\mathrm{th}}$ order and conforms to the \text{DUNE}
\citeDune grid API. We expect the reader to be at least briefly familiar with the Dune interface to fully benefit from this paper.
%%
...
%%
%%
\textbf{(2)} once the mesh has been read from disk, it is partitioned using the \texttt{Parmetis} \citeParMetis
...
%%
%%
The \texttt{dune-curvilineargrid} grid manager is continuously developed and improved. Among other things, we are working
...
%%
%%
As more work is being done on \texttt{dune-curvilineargrid}, this document will be updated. For the most recent version, please contact the \textbf{[REPOSITORY]}
...
\bibliography{bib/oswald,bib/numerical_libraries,bib/finite_element_method,bib/electromagnetic}
